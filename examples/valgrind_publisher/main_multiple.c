...
#else
#define MIN_BUFFER_SIZE 1000 //144 Bytes aprox
...
int main(int args, char** argv)
{
    ...
    uint32_t num_pub = (uint32_t)atoi(argv[2]);
    ...
    uxrObjectId * datawriter_id_array = (uxrObjectId *)malloc(num_pub*sizeof(uxrObjectId));
#ifdef USE_XML_CREATION
    // Create entities
    ...
    {
        uxrObjectId participant_id = uxr_object_id(i+1, UXR_PARTICIPANT_ID);
        ...
        uxrObjectId topic_id = uxr_object_id(i+1, UXR_TOPIC_ID);
        ...
        datawriter_id_array[i] = datawriter_id;
    ...
    {   
        for (size_t i = 0; i < num_pub; i++)
        {
            ...
            uxr_prepare_output_stream(&session, reliable_out, datawriter_id_array[i], &ub, topic_size);

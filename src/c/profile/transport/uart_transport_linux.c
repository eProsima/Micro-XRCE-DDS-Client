...
intmax_t read_serial_data(void* transport, uint8_t* buf, size_t len);
...
{
    intmax_t result = write_serial_msg(&transport->serial_io, buf, len, transport->addr);
    if (0 < result)
    {
        result = write(transport->fd, transport->serial_io.output_buffer, (size_t)result);
...
{
    (void)timeout;
    intmax_t result = read_serial_msg((void*)transport, read_serial_data , transport->buffer, sizeof(transport->buffer));
    if (0 < result)
    {
        *len = result;
